...
export class Utils {
	...
	static findClosestPointOnTheLineToTheCenterOfCircle(
		circle: Circle,
		line: LineI,
	): PointI {
		const componentOne =
			(circle.center.x - line.start.x) * (line.end.x - line.start.x) +
			(circle.center.y - line.start.y) * (line.end.y - line.start.y);
		const componentTwo = Math.pow(
			Utils.distanceBetweenTwoPoints(line.start, line.end),
			2,
		);
